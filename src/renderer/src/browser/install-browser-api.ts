...
import { preprocessRequest } from '../../../main/fragments/request'
  ...
  try {
    ...
    while (item) {
      ...
      try {
        ...
        const capabilities = modelCapabilities(item.request.model)
        const selectedVibes = !capabilities.vibes
          ? []
        ...
        const selectedCharRefs = !capabilities.characterReferences
          ? []
        ...
        const result = await gateway<{
          ...
          {
            token: account.token,
            request: preprocessRequest(item.request, browserFragmentSource(state)),
...
async function dispatch(channel: string, rawRequest: unknown): Promise<unknown> {
  ...
  if (channel === 'frags:resetSequential') {
  ...
  if (channel === 'tokens:count') {
    const source = browserFragmentSource(await readBrowserState())
    return gateway('tokens', {
      ...request,
      texts: (request.texts as string[]).map((text) =>
        processWildcards(removeComments(text), source, () => 0, true)
      )
    })
  }
  ...
  if (channel === 'images:readForSource') {
    const item = findBrowserImage(await readBrowserState(), request.filePath)
  ...
  if (channel === 'images:readMetadata') {
    const state = await readBrowserState()
    const item = findBrowserImage(state, request.filePath)
    if (!item) return { error: 'Metadata is unavailable for this image' }
    const sent =
      'payloadJson' in item && item.payloadJson
    ...
    return {
      meta: {
        prompt: sent?.input ?? '',
        negativePrompt: '',
        seed: 'seed' in item ? (item.seed ?? undefined) : undefined,
  ...
  if (channel === 'images:analyzeArtists') {
    const state = await readBrowserState()
    const stored = findBrowserImage(state, request.filePath)
  ...
  if (channel === 'images:upscale') {
    ...
    try {
      ...
      const filePath = await saveBrowserImage(
        result.base64,
        'upscale',
        JSON.stringify({ upscale: UPSCALE_SCALE, model: UPSCALE_MODEL })
      )
  ...
  if (channel === 'backup:export') {
    const state = await readBrowserState()
    download(
      btoa(unescape(encodeURIComponent(JSON.stringify(exportBrowserWorkspace(state), null, 2)))),
  ...
  if (channel === 'backup:import') {
    ...
    try {
      ...
      await mutateBrowserState((state) => importBrowserWorkspace(state, parsed))
      resetSequentialCounters()
