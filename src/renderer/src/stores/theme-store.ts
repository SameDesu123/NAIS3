...
import { isLang, LANGUAGES } from '@shared/i18n'
...
function stacksForLang(lang: string): { ui: string; mono: string } {
  const fallback = isLang(lang) ? LANGUAGES[lang].fontFallback : ''
  const prefix = fallback ? `${fallback}, ` : ''
  return {
    ui: `'Pretendard Variable', ${prefix}${SYSTEM_UI}, ${KOREAN_UI}, sans-serif`,
    mono: `'JetBrains Mono', Consolas, ui-monospace, 'Pretendard Variable', ${prefix}${KOREAN_UI}, monospace`
  }
}
