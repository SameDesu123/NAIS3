 ...
 *   (b) 완전 새 설치(DB 파일을 이번 실행에 생성함) → 그때만 OS 로케일을 따른다.
 * 한 번 정해지면 값이 남으므로 이 판정은 다시 실행되지 않는다.
 */
export function resolveInitialLanguage(isNewDatabase = false): Lang {
  const existing = getSetting('ui_language')
  if (isLang(existing)) return existing

  let lang: Lang = 'ko'
  try {
    if (isNewDatabase) lang = localeToLang(app.getLocale())
  } catch {
    // 판정 불가 = 안전한 쪽(한국어)으로
  }
